...
def TD(state_map, trans_matrix, Vd, STEP, GAMMA):
    Ve = state_map.copy()
    Ve = dict.fromkeys(Ve, 0)
    ...
    while error > 1:
        ...
        next_possible_states = trans_matrix[current_state][str(current_action)]
        next_state = np.random.choice(list(next_possible_states), p=list(next_possible_states.values()))

        rew = reward(state_map, next_state)

        Ve[current_state] = Ve[current_state] + STEP * (rew + GAMMA * Ve[next_state] - Ve[current_state])

        error = np.abs(Ve["000000000"] - Vd["000000000"])
        # error = np.linalg.norm(np.fromiter(Ve.values(), dtype=float) - np.fromiter(Vd.values(), dtype=float))
        # if(i % 10000 == 0):
        #     print("\nIteration", i, ": ")
        #     print(current_state, " -> ", current_action, " ", rew, " -> ", next_state)
        #     print("Error: ", error)
    ...
    with open('estimated_value.json', 'w') as f:
...
# Loading policy from policy.json file. To recreate it uncomment the below line.
# create_policy(trans_matrix)
with open('policy.json') as f: 
    policy = json.load(f)

# Evaluating the above policy using policy evaluation function.
# policy_evaluation(policy, state_map, trans_matrix, 0.85, 0.01)
with open('value.json') as f: 
    Vd = json.load(f)

# Finding the Value function (Ve) using temporal learning and comparing it with actual value function (Vd).
TD(state_map, trans_matrix, Vd, 0.2, 0.85)
