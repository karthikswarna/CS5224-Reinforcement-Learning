...
def create_policy(trans_matrix):
    ...
    for state in trans_matrix:
        ...
        for index, pos in enumerate(state):
            if pos == '0':
                if c == selected_index:
                    policy[state] = index           # Select selected_index'th empty position in board(i.e '0') as action.
                    break
                c = c + 1
...
def value_iteration(state_map, trans_matrix, GAMMA, EPSILON):
    converged = False
    V = state_map.copy()
    ...
    while not converged:
        for state in state_map:
            maxValue = float('-inf')
            bestAction = random.choice(list(trans_matrix[state].keys()))    # Randomly initialize bestAction from valid actions.

            # Maxing out reward over all the actions.
            for action, transitions in trans_matrix[state].items():
                ...
                # If the expected reward is better, update the value function.
    ...
    with open('policy_value_iter.json', 'w') as f:
...
value_iteration(state_map, trans_matrix, 0.85, 0.01)
with open('policy_value_iter.json') as f: 
    policy = json.load(f)
